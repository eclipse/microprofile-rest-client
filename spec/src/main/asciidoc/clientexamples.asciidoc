...
Implementations must throw a RestClientDefinitionException if an asynchronous method that accepts an InvocationCallback parameter also has a non-void return type.  For example:
[source, java]
----
@Path("/somePath")
public interface BadAsyncInterface {
    @GET
    public Response getEntry(InvocationCallback<SomeEntry> callback);
}
----
