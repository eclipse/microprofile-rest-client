...
A method is considered to be asynchronous if either of the following conditions are true:

- The method's return type is `java.util.concurrent.Future`.
- The method's return type is `void` but it has a parameter of type `javax.ws.rs.client.InvocationCallback`.
...
public interface MyAsyncClient {
    ...
    @POST
    @Path("/two")
    Future<String> post(String entity);

    @PUT
    @Path("/three")
    void put(InvocationCallback<JSONObject> callback, String updatedEntity);
}
----
